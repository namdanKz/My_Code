...
#test save log
fTestname = f"expTest.dat"
print(fTestname)
open(fTestname,'w').close()
NewLine = '\n'
...
class myNode():
    def __init__(self, nodeid, bs, period, packetlen):
        ...
        rounds = 0
        global nodes
        while (found == 0 and rounds < 100): #100
            a = random.random()
            b = random.random()
            if b<a:
                a,b = b,a
...
def transmit(env,node):
    while True:
        ...
        txt_log = f"***** node id = {node.nodeid} start sent time = {env.now} {NewLine}"
        print(txt_log)
        with open(fTestname, "a") as myfile:
            myfile.write(txt_log)
        myfile.close()
        ...
        txt_log = f"***** node id = {node.nodeid} finish sent time = {node.packet.rectime+env.now} {NewLine}"
        print(txt_log)
        with open(fTestname, "a") as myfile:
            myfile.write(txt_log)
        myfile.close()
...
#coment is original value
Ptx = 14  #14
gamma = 2.08 #2.08
d0 = 40.0 #40.0
var = 0       #0    # variance ignored for now
Lpld0 = 127.41 #127.41
GL = 0 #0
...
#maxDist = 200 #test Maximum distance 
...
for i in range(0,nrNodes):
    ...
    # 1000000 = 16 min
    node = myNode(i,bsId, avgSendTime,10) #packetlen original = 20
    nodes.append(node)
